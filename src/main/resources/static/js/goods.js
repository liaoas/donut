    ...
    let table = new Vue({
        ...
        methods: {
            ...
            popup: function (goods) {
                ...
                for (let i = 0; i < goods.dImgs.length; i++) {
                    console.log(goods.dImgs[i].img);
                    cloudArr.push(goods.dImgs[i].img);
                }
                this.updategGoods.img = cloudArr;
                console.log("我是清除图片后的数组" + this.updategGoods)
                // 弹出框事件
                $('#itemUser').popup({
                    time: 1000,
            ...
            updateUserEvent: async function (type) {
                ...
                if (type) {
                    ...
                    console.log(this.updategGoods.id);
                    console.log(this.updategGoods.name);
                    console.log(this.updategGoods.introduction);

                    // 执行产品修改
                    params.append('productId', this.updategGoods.id);
                    // 执行产品图片删除
                    let delImg = await axios.post('img/del', params);
                    console.log("我是产品图片删除" + delImg);
                    if (delImg.bool) {
                        // 删除成功
                        // 执行商品修改
                        params.append('id', this.updategGoods.id);
                        params.append('productName', this.updategGoods.name);
                        params.append('productIntroduction', this.updategGoods.introduction);
                        params.append('productStatus', this.updategGoods.status);
                        let updatePro = await axios.post('product/update', params);
                        console.log("我是产品修改" + updatePro);
                        // 执行产品图片添加
                        params.append('imgList[]', this.updategGoods.img);
                        params.append('productId', this.updategGoods.id);
                        let addProimh = await axios.post('img/add', params);
                        if (addProimh.bool == true && updatePro.bool == true) {
                            spop({
                                template: '产品修改成功',
                                group: 'submit-satus',
                                style: 'success',
            ...
            // 产品删除图标
            delIconClick:async function (id) {
                this.delId = id;
                // 弹出框事件
                $('#delGoods').popup({
                    time: 1000,
            ...
            // 是否确认删除按钮单击事件
            goodsDel: async function (type) {
                // 确认删除
                if (type){
                    if (this.delId == null || this.delId == '') {
                        spop({
                            template: '获取产品失败',
                            group: 'submit-satus',
                            style: 'error',
                    ...
                    let delPro = await axios.post('product/del', params);
                    if (delPro.bool){
                        spop({
                            template: delPro.message,
                            group: 'submit-satus',
                            style: 'success',
                            autoclose: 2000
                        });
